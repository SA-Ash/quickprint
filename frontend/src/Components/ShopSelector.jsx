...
const ShopSelector = ({ onShopSelect, selectedShop, userLocation }) => {
  ...
  const loadShops = async () => {
    if (userLocation && userLocation.lat && userLocation.lng) {
      await getNearbyShops(userLocation.lat, userLocation.lng, 5000);
    } else {
      ...
      if (storedLocation) {
        try {
          ...
          if (location.lat && location.lng) {
            await getNearbyShops(location.lat, location.lng, 5000);
            return;
          }
        } catch (error) {
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    {shop.distance && (
                      <div className="flex items-center text-gray-600 text-xs sm:text-sm">
                        ...
                        <span>{(shop.distance / 1000).toFixed(1)} km away</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="text-right">
                    <div className="text-xs text-gray-500">
                      ...
                      {shop.capacity?.maxQueue || 10}
                    </div>
                    <div className={`text-xs font-medium ${shop.isActive ? 'text-green-600' : 'text-red-600'}`}>
